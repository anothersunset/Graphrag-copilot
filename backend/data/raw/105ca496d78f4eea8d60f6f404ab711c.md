...
- **输出字段**: intent（意图）、entities（实体）、keywords（关键词）、complexity（复杂度）、requires_multi_hop（是否多跳）、query_rewrite（改写后的查询）、original_query（原始查询）
- **意图类型**: query, compare, summarize, analyze, recommend
...
- **特殊处理**: 当 context 为空时，直接返回「当前知识库中没有找到足够信息回答这个问题」，confidence=0.0，limitations=retrieval_empty，不调用 LLM
...
- **特殊处理**: 当 confidence < 0.6 时，添加警告提示「当前答案置信度较低，建议结合原文进一步核实」
- **附加**: 会附加 VerificationAgent 发现的 issues
...
- **流程**:
  1. Phase 1: 问题理解 (QueryUnderstandingAgent.analyze)
  2. Phase 2: 混合检索 (RetrievalAgent.hybrid_search)
  3. Phase 3: 流式推理 (StreamingReasoningAgent.reason_stream) 逐 token 输出
  4. Phase 4: 校验 (VerificationAgent.verify)
- **SSE 事件类型**: phase, answer_start, token, answer_end, done

### 3.2 StreamingReasoningAgent
- **方法**: reason_stream(query, context, analysis) → Generator[str]
- **特殊处理**: 当 context 为空时，yield「当前知识库中没有找到足够信息来回答这个问题」
...
- **chat(messages)**: 普通对话
- **chat_stream(messages)**: 流式对话，逐 token yield
- **chat_json(messages)**: JSON 格式输出
- **extract_entities(text)**: 实体抽取，支持 Person/Organization/Product/Technology/Concept/Document/Event/Location/Entity 类型
- **verify_answer(question, answer, sources)**: 答案验证

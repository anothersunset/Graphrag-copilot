...
export function AuditDrawer({
  ...
  const traceRows = run.retrieval_trace.filter(
    (r) => activeNode !== "retriever" || r.source
  );
  return (
    <aside className="w-96 shrink-0 rounded-2xl border bg-card p-4 shadow-sm overflow-y-auto">
      ...
      {activeNode === "retriever" && (
        <section>
          <h3 className="text-xs uppercase tracking-wider text-muted-foreground mb-1">retrieval trace</h3>
          <ul className="space-y-1">
            {traceRows.slice(0, 10).map((r) => (
              ...
              >
                <span className="font-mono">{r.source}·{r.chunk_id.slice(0, 12)}</span>

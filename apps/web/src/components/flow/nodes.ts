...
export function buildLayout(
	...
	const layouted = dagreLayout(visible, edges, direction);
...
function buildEdges(
	...
	if (has("planner") && has("retriever"))
		out.push(edge("planner", "retriever"));
	if (has("retriever") && has("evaluator"))
		out.push(edge("retriever", "evaluator"));
	if (has("evaluator") && has("rewriter"))
		out.push(edge("evaluator", "rewriter", { label: "rewrite" }));
	if (has("rewriter") && has("retriever"))
		out.push(edge("rewriter", "retriever", { label: "loop" }));
	if (has("evaluator") && has("generator"))
		out.push(edge("evaluator", "generator", { label: "use" }));
	if (has("evaluator") && has("fallback"))
		out.push(edge("evaluator", "fallback", { label: "fallback" }));
	if (has("generator") && has("auditor"))
		out.push(edge("generator", "auditor"));
	if (has("fallback") && has("auditor")) out.push(edge("fallback", "auditor"));

	const cited = new Set(
		run?.retrieval_trace.filter((r) => r.cited).map((r) => r.source) ?? [],
	);
	return out.map((e) => ({
		...e,
		data: {
			...(e.data ?? {}),
			cited: cited.size > 0 && /retriever|generator|auditor/.test(e.target),
		},
	}));
}
...
function dagreLayout(
	...
	g.setGraph({
		rankdir: direction,
		nodesep: direction === "TB" ? 20 : 32,
		ranksep: direction === "TB" ? 34 : 56,
	});
...
function nodeStyle(id: string, run: RunResponse | null) {
	const decision = run?.audit.find((a) => a.node === "evaluator")?.detail
		?.crag_decision;
...
const accent = () => ({ ...base(), border: "1px solid var(--color-primary)" });
const warn = () => ({
	...base(),

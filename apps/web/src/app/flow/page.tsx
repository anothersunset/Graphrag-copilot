...
async function FlowContent({ run }: { run?: string }) {
  if (!run) {
    return (
      <div className="rounded-2xl border bg-card p-6 text-sm text-muted-foreground">
        Add a <code className="rounded bg-muted px-1 py-0.5">?run=&lt;run-id&gt;</code> query to
        inspect a completed GraphRAG run.
      </div>
    )
  }
  ...
  try {
    ...
    if (!data) {
      return (
        <output className="block rounded-2xl border bg-card p-6 text-sm">
          Run <span className="break-all font-mono">{run}</span> was not found or has expired.
        </output>
      )
    }
    ...
    return (
      <div className="rounded-2xl border border-destructive bg-card p-6 text-sm" role="alert">
        Unable to load this run. {error instanceof Error ? error.message : String(error)}
      </div>
    )
  }
}
    ...
    <main className="min-h-screen bg-background p-4 text-foreground sm:p-6">
      <header className="mb-4 flex flex-wrap items-center justify-between gap-3">
        <h1 className="min-w-0 text-xl font-semibold sm:text-2xl">
          Agent run ·{" "}
          <span className="break-all font-mono text-base sm:text-lg">{run ?? "new"}</span>
        </h1>
        <a

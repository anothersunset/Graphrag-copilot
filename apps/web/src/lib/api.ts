...
export interface RunResponse {
  ...
  tool_calls: Array<{ tool?: string; name?: string; args?: Record<string, unknown> }>
...
export async function fetchRun(id: string): Promise<RunResponse | null> {
  const response = await fetch(`${API_BASE}/v1/runs/${encodeURIComponent(id)}`, {
    cache: "no-store",
  })
  if (response.status === 404) return null
  return readJson<RunResponse>(response, "fetch run")
}

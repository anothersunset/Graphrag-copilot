...
def test_rerank_reorders_by_injected_scores():
    ...
    fake_scorer = lambda q, contents: [0.6 if c == "cats" else 0.2 if c == "dogs" else 0.95 for c in contents]
...
def test_rerank_topk_truncation():
    hits = [
        {"chunk_id": str(i), "content": f"doc {i}", "score": 0.5, "source": "vector", "metadata": {}}
        for i in range(10)
    ]

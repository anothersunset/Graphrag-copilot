...
Uses a typed Signature:
    question, contexts, draft_answer
      -> verdict ("pass" | "hallucination" | "unsupported"),
         rationale (str),
         cited_chunk_ids (list[str])

The LM is dependency-injected so tests + offline runs work without an API
key. If DSPy isn't installed, ``DSPyAuditor.audit`` raises and the
auditor node falls back to the citation-count heuristic.
"""
    ...
    def audit(
        ...
        *,
        ...
        try:
            ...
            return AuditVerdict(verdict="unsupported", rationale="auditor invocation failed", cited_chunk_ids=[])
        ...
        return AuditVerdict(verdict=verdict, rationale=rationale, cited_chunk_ids=list(cited))
